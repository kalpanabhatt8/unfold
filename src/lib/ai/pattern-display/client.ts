...
const inflight = new Map<string, Promise<PatternDisplay>>();

const inflightKey = (name: PatternName, evidenceKey: string): string =>
  `${name}|${evidenceKey}`;
...
export async function fetchPatternDisplay(
  ...
  const key = inflightKey(input.name, input.evidenceKey);
  const existing = inflight.get(key);
  if (existing) return existing;

  const promise = (async (): Promise<PatternDisplay> => {
